...
import { MapPin, Link as LinkIcon, Code, Users, Briefcase, Award, Zap, ChevronLeft, ZapIcon, ExternalLink, Trash2, ThumbsUp, MessageSquare, UserPlus, Handshake, Star, Settings, Network } from 'lucide-react';
...
const ProfilePage = () => {
  ...
  if (loading) return <div className="min-h-screen bg-[#050810] text-blue-400 flex items-center justify-center font-mono animate-pulse uppercase tracking-widest">Fetching_Profile_Vectors...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#050810] text-gray-200 font-sans selection:bg-blue-500/30 pb-20">
      ...
      {isOwnProfile && (
        <>
          <EditProfileModal profile={profile} isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onUpdate={(updatedProfile) => setProfileData({ ...profileData, profile: updatedProfile })} />
          <AddPortfolioModal isOpen={isPortfolioOpen} onClose={() => setIsPortfolioOpen(false)} onUpdate={(updatedPortfolio) => setProfileData({ ...profileData, profile: { ...profile, portfolio: updatedPortfolio } })} />
        </>
      )}
      ...
      <header className="relative max-w-5xl mx-auto mt-6 rounded-3xl overflow-visible border border-white/10 bg-[#0a0f1c] shadow-[0_0_40px_rgba(37,99,235,0.1)]">
        ...
        {/* NEW: TOP RIGHT ICONS FOR ACCOUNT HOLDER */}
        {isOwnProfile && (
          <div className="absolute top-4 right-4 z-20 flex items-center gap-3">
            <Link to="/network" className="p-2.5 bg-black/50 hover:bg-black/80 text-white rounded-full backdrop-blur-md transition border border-white/10 shadow-lg" title="Network Core">
              <Network size={20} />
            </Link>
            ...
            <div className="relative">
              <button 
                ...
                className={`p-2.5 rounded-full backdrop-blur-md transition border shadow-lg ${isSettingsOpen ? 'bg-blue-600 border-blue-500 text-white' : 'bg-black/50 hover:bg-black/80 border-white/10 text-white'}`}
                title="Settings"
              >
              ...
              {/* Settings Dropdown */}
              {isSettingsOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-[#0a0f1c]/95 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-2">
                  <button 
                    onClick={() => { setIsEditOpen(true); setIsSettingsOpen(false); }} 
                    className="w-full text-left px-5 py-3 text-sm font-bold text-gray-300 hover:bg-white/5 hover:text-white transition"
                  >
                    Edit Parameters
                  </button>
                </div>
        ...
        {/* BANNER BACKGROUND */}
        <div className="h-48 md:h-64 w-full bg-gradient-to-r from-blue-900 to-purple-900 relative rounded-t-3xl overflow-hidden">
        ...
        <div className="px-8 pb-8 relative -mt-20 flex flex-col md:flex-row items-center md:items-start gap-6 text-center md:text-left">
          ...
          <div className="flex-1 mb-2 z-10 mt-2">
            ...
            {profile?.username && <p className="text-blue-500 font-mono text-sm tracking-widest mb-1">@{profile.username}</p>}
            ...
            {/* NEW: VISITOR ACTION BUTTONS UNDER HEADLINE */}
            {!isOwnProfile && (
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 mt-6">
                <button onClick={handleFollowToggle} className={`flex items-center justify-center gap-2 px-6 py-2.5 rounded-xl text-xs font-bold transition ${isFollowing ? 'bg-white/10 text-white hover:bg-red-500/20 hover:text-red-400 border border-white/10' : 'bg-yellow-500 hover:bg-yellow-400 text-black shadow-[0_0_15px_rgba(234,179,8,0.3)]'}`}>
                  <UserPlus size={16} /> {isFollowing ? 'Unfollow' : 'Follow Node'}
                </button>
                <button onClick={handleConnectRequest} className="flex items-center justify-center gap-2 bg-white hover:bg-gray-200 text-black px-6 py-2.5 rounded-xl text-xs font-bold transition shadow-[0_0_15px_rgba(255,255,255,0.2)]">
                  <Handshake size={16} /> Connect
                </button>
                <Link to="/messages" className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-2.5 rounded-xl text-xs font-bold transition shadow-[0_0_15px_rgba(37,99,235,0.4)]">
      ...
      <main className="max-w-5xl mx-auto mt-8 grid grid-cols-1 lg:grid-cols-3 gap-8 px-4 md:px-0">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-[#050810] border border-white/10 overflow-hidden flex items-center justify-center font-bold text-white text-xs">
                          {review.reviewer?.profilePictureUrl ? <img src={`https://bizferbine-backend.onrender.com/${review.reviewer.profilePictureUrl}`} className="w-full h-full object-cover" /> : review.reviewer?.name?.charAt(0)}
                        </div>
          ...
          {(profile?.socialLinks?.linkedIn || profile?.socialLinks?.github || profile?.socialLinks?.website) && (
            <section className="bg-[#0a0f1c] border border-white/10 rounded-3xl p-6">
              ...
              <div className="space-y-3">
                {profile.socialLinks.linkedIn && <a href={profile.socialLinks.linkedIn} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-blue-400 transition"><Users size={16} /> LinkedIn Profile</a>}
                {profile.socialLinks.github && <a href={profile.socialLinks.github} target="_blank" rel="noreferrer" className="flex items-center gap-3 text-sm text-gray-300 hover:text-purple-400 transition"><Code size={16} /> GitHub Repository</a>}
              </div>
            </section>
          )}
        </div>
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {profile.portfolio.map((item, index) => (
                  <div key={item._id || index} className="bg-black border border-white/5 rounded-2xl overflow-hidden group flex flex-col relative hover:border-blue-500/30 transition-colors">
                    {isOwnProfile && <button onClick={() => handleDeletePortfolio(item._id)} className="absolute top-2 right-2 p-2 bg-black/60 text-gray-400 hover:text-red-400 rounded-full transition z-20 opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>}
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      {item.result && <div className="mt-auto mb-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs p-2 rounded-lg font-mono">Output: {item.result}</div>}
                      <div className="flex gap-3 text-xs mt-auto">
                        {item.projectUrl && <a href={item.projectUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-blue-400 hover:underline"><ExternalLink size={12}/> Live</a>}
                        {item.githubUrl && <a href={item.githubUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-purple-400 hover:underline"><Code size={12}/> Source</a>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <section className="bg-[#0a0f1c] border border-white/10 rounded-3xl p-8">
             <h2 className="text-white font-bold mb-6 flex items-center gap-2 uppercase tracking-widest text-[10px] font-mono text-gray-500"><ZapIcon size={14} className="text-yellow-400" /> Thought Leadership</h2>
                ...
                {thoughtLeadershipFeed.map((insight) => {
                  ...
                  return (
                    <div key={insight._id} className="bg-black border border-white/5 rounded-2xl p-5 relative group">
                      {isOwnProfile && <button onClick={() => handleDeleteInsight(insight._id)} className="absolute top-4 right-4 p-2 text-gray-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition z-20"><Trash2 size={16} /></button>}
                      ...
                      <div className="flex items-center gap-6 pt-4 border-t border-white/5">
                        <div className="flex items-center gap-2 text-xs font-mono tracking-widest uppercase text-gray-500">
                          <ThumbsUp size={14} className={hasLiked ? "fill-blue-400/20 text-blue-400" : ""} /> 
                          <span className={hasLiked ? "text-blue-400" : ""}>{insight.likes?.length || 0} Likes</span>
                        </div>
                        ...
                        <Link to="/insights" className="ml-auto text-[10px] font-mono text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-full hover:bg-blue-500/10 transition uppercase tracking-widest">
                          Open in Feed →
                        </Link>
                      </div>
                    </div>
