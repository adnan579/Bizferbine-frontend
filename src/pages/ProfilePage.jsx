...
const ProfilePage = () => {
  ...
  // --- PHASE 1.5: SILENT OUTBOUND TRACKING ENGINE ---
  const trackOutboundClick = (eventType, metadata = {}) => {
    // We do NOT want to track a user clicking their own links!
    if (isOwnProfile) return; 

    // Fire and forget - no await, no loading spinners
    fetch('https://bizferbine-backend.onrender.com/api/analytics/track', {
      ...
      body: JSON.stringify({
        targetUser: targetProfileId,
        eventType: eventType,
        metadata: metadata
      })
    }).catch(err => console.error('Analytics ping failed', err)); // Silent fail
  };
  // ---------------------------------------------------
          ...
          {(profile?.socialLinks?.linkedIn || profile?.socialLinks?.github || profile?.socialLinks?.website) && (
            <section className="bg-[#0a0f1c] border border-white/10 rounded-3xl p-6">
              ...
              <div className="space-y-4 mt-2">
                {/* PHASE 1.5 WIRE UP: trackOutboundClick added to these 3 links */}
                {profile.socialLinks.website && (
                  <a href={profile.socialLinks.website} target="_blank" rel="noreferrer" 
                     ...
                     className="flex items-center gap-3 text-sm text-gray-300 hover:text-emerald-400 transition group">
                    <LinkIcon size={16} className="text-gray-500 group-hover:text-emerald-400 transition" /> Personal Website
                  </a>
                )}
                {profile.socialLinks.linkedIn && (
                  <a href={profile.socialLinks.linkedIn} target="_blank" rel="noreferrer" 
                     ...
                     className="flex items-center gap-3 text-sm text-gray-300 hover:text-blue-400 transition group">
                    <Users size={16} className="text-gray-500 group-hover:text-blue-400 transition" /> LinkedIn Profile
                  </a>
                )}
                {profile.socialLinks.github && (
                  <a href={profile.socialLinks.github} target="_blank" rel="noreferrer" 
                     ...
                     className="flex items-center gap-3 text-sm text-gray-300 hover:text-purple-400 transition group">
                    <Code size={16} className="text-gray-500 group-hover:text-purple-400 transition" /> GitHub Repository
                  </a>
                )}
              </div>
            </section>
                  ...
                  <div key={item._id || index} className="bg-black border border-white/5 rounded-2xl overflow-hidden group flex flex-col relative hover:border-blue-500/30 transition-colors">
                    ...
                    <div className="p-5 flex-1 flex flex-col">
                      ...
                      <div className="flex gap-3 text-xs mt-auto">
                        {/* PHASE 1.5 WIRE UP: trackOutboundClick added to these 2 buttons */}
                        {item.projectUrl && (
